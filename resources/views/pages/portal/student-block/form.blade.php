...
<div class="mb-5 form-group {{ $errors->has('student_number') ? 'has-error' : '' }}">
    ...
    <div class="col-md-12">
        <textarea class="form-control" name="student_number" placeholder="Example; 202381892, 2022987182" required>{{ old('student_number', optional($studentBlock)->student_number) }}</textarea>
    </div>
</div>
<div class="mb-5 form-group {{ $errors->has('reason') ? 'has-error' : '' }}">
    ...
    <div class="col-md-12">
        <textarea class="form-control" name="reason" placeholder="Enter reason here..." required>{{ old('reason', optional($studentBlock)->reason) }}</textarea>
    </div>
</div>