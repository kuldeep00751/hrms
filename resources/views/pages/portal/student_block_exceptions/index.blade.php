    ...
    <div class="col-md-10 mx-auto">
        <div class="card">
            <div class="card-header">
                <div class="pull-left">
                    <h3>Student Block Exceptions</h3>
                </div>
                <div class="pull-right">
                    <a href="{{ route('student_block_exceptions.student_block_exception.create') }}" class="btn btn-sm btn-light btn-active-light-primary">
                        <i class="fa-solid fa-plus"></i> Add New
                    </a>
                    <a href="{{ route('student_block_exceptions.student_block_exception.bulk-remove') }}" data-bs-toggle="modal" data-bs-target="#bulk-remove" class="btn btn-sm btn-light btn-active-light-danger">
            ...
            @if(count($studentBlockExceptions) == 0)
            <div class="card-body text-center">
                <h6>No Student Block Exceptions Available.</h6>
            </div>
            @else
            <div class="card-body">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-row-dashed" id="kt_datatable_example">
                        ...
                        <tbody>
                            @foreach($studentBlockExceptions as $studentBlockException)
                            <tr>
                                <td>{{ $studentBlockException->userInfo->student_number }}</td>
                                <td>{{ $studentBlockException->userInfo->first_names }} {{ $studentBlockException->userInfo->surname }}</td>
                                <td>{{ $studentBlockException->reason }}</td>
                                <td>{{ $studentBlockException->addedBy->first_name }} {{ $studentBlockException->addedBy->last_name }}</td>
                                <td>{{ $studentBlockException->batch_number }}</td>
    ...
    <div class="modal fade modal-dialog-scrollable" tabindex="-1" id="bulk-remove" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content shadow-none">
                ...
                <form method="POST" action="{{ route('student_block_exceptions.student_block_exception.bulk-remove') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                    {{ csrf_field() }}
