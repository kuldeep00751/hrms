    ...
    <div class="col-md-12 mb-5">
        <div class="card">
            <div class="card-header">
                <strong>Search Final Mark Grading Scale: </strong>
            </div>
            <form method="GET" action="{{ route('final_mark_criterias.final_mark_criteria.filter') }}" accept-charset="UTF-8" class="form-horizontal">
                <div class="card-body">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <a href="/final_mark_criterias" class="btn btn-white btn-active-light-primary me-2">{{ __('Reset') }}</a>
            ...
            <div class="card">
                ...
                <div class="card-header">
                    ...
                    <div class="pull-right" role="group">
                        <a href="{{ route('final_mark_criterias.final_mark_criteria.create') }}" class="btn btn-sm btn-primary" title="Create Final Mark Grading Scale">
                            <i class="fa-solid fa-plus"></i> Add New
                        </a>
                        <a href="#" class="btn btn-sm btn-light-primary" data-bs-toggle="modal" data-bs-target="#copy">
                            <i class="fa-solid fa-copy"></i>
                            Copy Final Mark Grading Scale
                        </a>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-row-dashed" >
                            ...
                            <tbody>
                                @forelse($finalMarkCriterias as $finalMarkCriteria)
                                <tr>
                                    <td>{{ $finalMarkCriteria->module->module_name }}</td>
                                    <td>{{ $finalMarkCriteria->module->module_code }}</td>
                                    <td class="text-center">{{ $finalMarkCriteria->academicYear->name }}</td>
                                    <td>{{ optional($finalMarkCriteria->assessmentType)->assessment_type }}</td>
                                    <td>{{ $finalMarkCriteria->min_mark }} - {{ $finalMarkCriteria->max_mark }}</td>
                                    <td>{{ $finalMarkCriteria->assessmentResultCode->result_code }} - {{ $finalMarkCriteria->assessmentResultCode->result_code_description }}</td>
                                    <td class="text-center">{{ $finalMarkCriteria->assessmentResultCode->pass_fail }} </td>
                                    <td>
                                        <a href="{{ route('final_mark_criterias.final_mark_criteria.edit', $finalMarkCriteria->id ) }}" class="btn btn-sm btn-light btn-active-light-primary">Edit</a>
                                    </td>
                                </tr>
                                @empty
                                <tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content shadow-none">
                <div class="modal-header">
                    <h5 class="modal-title">Copy Final Mark Grading</h5>
                ...
                <form method="POST" action="{{ route('final_mark_criterias.final_mark_criteria.copy') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                    {{ csrf_field() }}
