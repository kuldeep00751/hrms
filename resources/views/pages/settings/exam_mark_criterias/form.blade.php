    ...
    <div class="col-md-12">
        <div class="form-group {{ $errors->has('module_id') ? 'has-error' : '' }}">
            ...
            <select class="form-control grading-parameter" id="module_id" name="module_id" data-control="select2" @if($operation=='edit' ) disabled @endif required>
                <option value="" style="display: none;" {{ old('module_id', optional($examMarkCriteria)->module_id ?: '') == '' ? 'selected' : '' }} disabled selected>Select module</option>
                @foreach ($modules as $key => $module)
                <option value="{{ $key }}" {{ old('module_id', optional($examMarkCriteria)->module_id) == $key ? 'selected' : '' }}>
                    {{ $module }}
                </option>
                @endforeach
            </select>
    ...
    <div class="col-md-12">
        <div class="form-group {{ $errors->has('academic_year_id') ? 'has-error' : '' }}">
            ...
            <select class="form-control grading-parameter" id="academic_year_id" name="academic_year_id" @if($operation=='edit' ) disabled @endif required>
                <option value="" style="display: none;" {{ old('academic_year_id', optional($examMarkCriteria)->academic_year_id ?: '') == '' ? 'selected' : '' }} disabled selected>Select academic year</option>
                @foreach ($academicYears as $key => $academicYear)
                <option value="{{ $key }}" {{ old('academic_year_id', optional($examMarkCriteria)->academic_year_id) == $key ? 'selected' : '' }}>
                    {{ $academicYear }}
                </option>
                @endforeach
            </select>
    ...
    <div class="col-md-12">
        <div class="form-group {{ $errors->has('assessment_type_id') ? 'has-error' : '' }}">
            ...
            <select class="form-control grading-parameter" id="assessment_type_id" name="assessment_type_id" @if($operation=='edit' ) disabled @endif required>
                <option value="" style="display: none;" {{ old('assessment_type_id', optional($examMarkCriteria)->assessment_type_id ?: '') == '' ? 'selected' : '' }} disabled selected>Select Exam Type</option>
                @foreach ($assessmentTypes as $key => $assessmentType)
                <option value="{{ $key }}" {{ old('assessment_type_id', optional($examMarkCriteria)->assessment_type_id) == $key ? 'selected' : '' }}>
                    {{ $assessmentType }}
                </option>
                @endforeach
            </select>
...
<div class="row mb-5">
    <div class="col-md-12">
        ...
        <table class="table table-row-dashed">
            ...
            @else
            <tbody id="criteria-table">
                @foreach($examMarkCriterias as $examMarkCriteria)
                <tr>
                    <td class="align-middle">
                        <input class="form-control" name="min_mark[]" type="number" value="{{ $examMarkCriteria->min_mark }}" placeholder="0.0" required>
                    </td>
                    <td class="align-middle">
                        <input class="form-control" name="max_mark[]" type="number" value="{{ $examMarkCriteria->max_mark }}" placeholder="0.0" required>
                    </td>
                    <td class="align-middle">
                        <select class="form-control" name="assessment_result_code_id[]" required>
                            <option value="" style="display: none;" {{ old('assessment_result_code_id', optional($examMarkCriteria)->assessment_result_code_id ?: '') == '' ? 'selected' : '' }} disabled selected>Select result code</option>
                            @foreach ($assessmentResultCodes as $key => $assessmentResultCode)
                            <option value="{{ $key }}" {{ $examMarkCriteria->assessment_result_code_id == $key ? 'selected' : '' }}>
