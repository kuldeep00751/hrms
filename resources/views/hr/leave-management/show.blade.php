<x-base-layout>
        ...
        <div class="card">
            <div class="card">
                <div class="card-header">
                    <div class="pull-left">
                        <a href="{{ route('leave-mananagements.leave-mananagement.index') }}" class="btn btn-sm btn-secondary btn-active-light-primary"><i class="fa-solid fa-chevron-left"></i> Leave</a>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="row mb-5">
                        <div class="col-md-12">
                            <div class="form-group {{ $errors->has('employee_id') ? 'has-error' : '' }}">
                                <label for="employee_id" class="control-label"><strong>Employee Number: </strong></label>

                                <span>{{ $leaveManage->employeeInfo->employee_number}}</span>
                            </div>
                        </div>
                    </div>
                    <div class="row mb-5">
                    ...
                    <div class="row mb-5">
                        ...
                        <div class="row mb-5">
                            <div class="col-md-12">
                                <div class="form-group {{ $errors->has('reason_leave_approval') ? 'has-error' : '' }}">
                                    <label for="reason_leave_approval" class="control-label">Reason For Leave Approval /   Rejection: <span class="text-danger"></span></label>

                                    <textarea class="form-control" name="reason_leave_approval" id="reason_leave_approval" required>{{ old('reason_leave_approval', $leaveManage->reason_leave_approval ?? '') }}</textarea>
                                    {!! $errors->first('reason_leave_approval', '<p class="help-block">:message</p>') !!}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-5">
                            <div class="col-md-12">
                                <div class="form-group {{ $errors->has('reason_leave_approval') ? 'has-error' : '' }}">
                                    <div class="pull-left">
                                        <span class="btn btn-success btn-active-light-primary approve-btn" data-id="{{ $leaveManage->id }}">Approved</span>
                                        <span class="btn btn-danger btn-active-light-primary reject-btn" data-id="{{ $leaveManage->id }}">Rejected </span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.approve-btn, .reject-btn').forEach(function(button) {
                button.addEventListener('click', function() {
                    ...
                    if(reason_leave_approval =='' || reason_leave_approval ==null){
                        Swal.fire({
                            text: "Give the reason for leave Approval / Rejection",
                            icon: "warning",
                    ...
                    const url = "{{ route('admin.employees.update-status') }}";
                    ...
                    fetch(url, {
                        ...
                        Swal.fire({
                            ...
                            window.location.href = "{{ route('leave-mananagements.leave-mananagement.index') }}";
                        });
                    })
                    .catch(error => {
                        // Handle error
        ...
        function openDocument(attachmentId, attachmentName) {
            ...
            window.open(`/employees/leave-management/display/${attachmentId}`, `${attachmentName}`, params);
        };
    </script>
</x-base-layout>